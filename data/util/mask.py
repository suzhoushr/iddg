...
def random_bbox_v2(img_shape=256, max_bbox_shape=200, min_bbox_shape=16):
    '''
    shape: (w, h)
    '''
    random_sz = np.random.randint(min_bbox_shape, max_bbox_shape+1)
    delta = img_shape - random_sz
    x1 = np.random.randint(0, delta+1)
    y1 = np.random.randint(0, delta+1)
    x2 = x1 + random_sz
    y2 = y1 + random_sz
