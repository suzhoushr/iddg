...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.num_classes = num_classes + 1  # Note: 0 is left to [None] label

        cond_embed_dim = inner_channel * 4
        self.cond_embed_dim = cond_embed_dim
        if self.num_classes is not None:
            self.label_emb = nn.Embedding(self.num_classes, cond_embed_dim)
    ...
    def forward(self, x, y=None):
        ...
        # pdb.set_trace()
        hs = []
        emb = torch.randn(x.shape[0], self.cond_embed_dim).to(x.device)
